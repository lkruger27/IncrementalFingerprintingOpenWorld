...
def experiment():
    ...
    for file in glob.glob("./SSH/*.dot"):
        mealy_machine = load_automaton_from_file(file, automaton_type='mealy')
        models.append(mealy_machine)
        names.append(file[10:-4])

    tests = 500000
    with open('./SSH/L#_results.csv','a') as f:
        f.write('model,eq_oracle,tests,states,equivalent,learning_queries,learning_steps,eq_queries,eq_steps\n')
    for s in range(30):
        random.seed(s)
...
def evaluation():
    df = pd.read_csv("./SSH/L#_results.csv")
    df['Inputs'] = 13
    df['Copies'] = 5
