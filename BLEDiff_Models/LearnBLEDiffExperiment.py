...
def experiment():
    models = []
    names = []
    for file in glob.glob("./BLEDiff_Models/*.dot"):
        mealy_machine = load_automaton_from_file(file, automaton_type='mealy')
        mealy_machine.make_input_complete()
        mealy_machine.minimize()
        models.append(mealy_machine)
        names.append(file)
    alphabet = models[0].get_input_alphabet()

    tests = 500000
    with open('./BLEDiff_Models/L#_results.csv','a') as f:
        f.write(f"model,oracle,tests,states,equivalent,learning_queries,learning_steps,eq_queries,eq_steps\n")
    for s in range(30):
        random.seed(s)
        for i in range(len(models)):
            ...
            with open('./BLEDiff_Models/L#_results.csv','a') as f:
                f.write(f"{names[i]},randomWp,{tests},{info['automaton_size']},{len(m.states)==info['automaton_size']},{info['queries_learning']},{info['steps_learning']},{info['queries_eq_oracle']},{info['steps_eq_oracle']}\n")

def evaluation():
    plt.rcParams['figure.constrained_layout.use'] = True
    df = pd.read_csv("./BLEDiff_Models/L#_results.csv")
    df['Inputs'] = 7
    df['Copies'] = 5
    df['Learning Symbols'] = df['learning_queries'] + df['learning_steps']
    df['Testing Symbols'] = df['eq_queries'] + df['eq_steps']
    df = df.groupby(['model']).mean(numeric_only=True)
    print(df.to_latex(columns=['model', 'states', 'Inputs', 'Copies', 'Learning Symbols', 'Testing Symbols'],float_format="%.0f" ))
