...
class StatePrefixOracleFailSafe(StatePrefixEqOracle):
    ...
    MAX_CEX_ATTEMPTS = 3
    MIN_CEX_ATTEMPTS = 2 # needs to be smaller than MAX CEX ATTEMPTS
    reset_time = 0
    physical_reset = 0
    ...
    def repeat_query(self, hypothesis, input_sequence):
        ...
        cex_found_counter = 0
        while cex_found_counter < self.MAX_CEX_ATTEMPTS:
            non_det_attempts = 0
            while non_det_attempts < constant.NON_DET_ERROR_ATTEMPTS:
                self.reset_hyp_and_sul(hypothesis)
                print(f"repeat {cex_found_counter}")
                for input in input_sequence:
                    ...
                    if out_sul != out_hyp:
                        cex_found_counter += 1
                        if cex_found_counter == self.MIN_CEX_ATTEMPTS:
                            return True
                        else:
                            break

            if non_det_attempts == constant.NON_DET_ERROR_ATTEMPTS and cex_found_counter < self.MAX_CEX_ATTEMPTS:
                return False
            ...
            while out_sul == constant.ERROR and error_counter < constant.CONNECTION_ERROR_ATTEMPTS and non_det_attempts < constant.NON_DET_ERROR_ATTEMPTS:
                try:
                    ...
                    for _ in range(self.steps_per_walk):
                        ...
                        if out_sul != out_hyp:
                            print("we found a potential cex")
