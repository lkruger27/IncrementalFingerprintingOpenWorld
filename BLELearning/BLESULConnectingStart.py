...
class BLESULConnectingStart(BLESUL):
    ...
    def pre(self):
        ...
        while physical_reset_attempts < constant.MAX_PHYSICAL_RESET:
        ...
        if physical_reset_attempts >= constant.MAX_PHYSICAL_RESET:
    ...
    def step(self, letter):
        """
        wrapper for step method to catch empty response

        Args:
            letter (string): input symbol
        """
        out = super().step(letter)

        if (constant.ALLOW_EMPTY_RSP == False and self.EMPTY == out):
