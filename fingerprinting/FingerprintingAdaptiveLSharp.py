...
def run_adaptive_Lsharp(alphabet: list, sul: SUL, references: list, eq_oracle: Oracle, automaton_type='mealy',
                        ...
                        cache_and_non_det_check=True, return_data=False, print_level=2, budget=None):
    """
    ...
    ob_tree = AdaptiveObservationTree(alphabet, sul, references, 'mealy', extension_rule, separation_rule, rebuilding, state_matching)
    ...
    if cache_and_non_det_check or samples is not None:
        ...
        if samples:
            for (input_seq, output_seq) in samples:
                ...
                ob_tree.insert_observation(input_seq, output_seq)
    ...
    while True:
        ...
        hypothesis_new = ob_tree.build_hypothesis()
        if (budget is None) or (budget and sul.num_steps + sul.num_queries < budget):
            hypothesis = hypothesis_new
        elif (budget != None) and (sul.num_steps + sul.num_queries) >= budget:
            if hypothesis is None:
                st = MealyState('s0')
                for i in alphabet:
                    st.transitions[i] = st
                    st.output_fun[i] = sul.query([i])
                hypothesis = MealyMachine(st, [st]) 
            sul.num_steps -= ((sul.num_steps + sul.num_queries) - budget)
            print('not enough budget, taking previous hyp')
            break
    ...
    if not hypothesis:
        ...
        hyp_size = hypothesis.size
    info = {
        'learning_rounds': learning_rounds,
        'automaton_size': hyp_size,
