...
df['LCQ_FP'] = 100 * df['learning_fp'] / df['implementations']
df['FCQ_FP'] = 100 * df['matching_fp'] / df['implementations']
df['Total_FP'] = 100 * (df['learning_fp'] + df['matching_fp']) / df['implementations']
...
df['oracle_type2'] = pd.Categorical(df['oracle_type2'], ["RandomWp25", "RandomWp50", "RandomWp100"])
df.sort_values("oracle_type2")
print(df)

df['benchmark'] = pd.Categorical(df['benchmark'], ["TLS", "SSH", "BLEDiff", "MQTT", "BLE"])
df.sort_values("benchmark")

g = sns.barplot(df, x="benchmark", y="LCQ_FP", hue="oracle_type2", errorbar=None)
sns.barplot(df, x="benchmark", y="Total_FP", hue="oracle_type2", alpha=0.5, zorder=-1, errorbar=None, ax=g)
# g.set_yscale("log")
g.set_ylabel("Misclassifications (%)",fontsize=13)
g.set_xlabel("Benchmark\n",fontsize=13)
plt.yticks(fontsize=12)
plt.xticks(fontsize=12)
legend_elements = [Line2D([0], [0], lw=0, color='k', marker='s', label='LCQ Misclassifications'),
                   Line2D([0], [0], lw=0, color='k', marker='s', alpha=0.5, label='FCQ Misclassifications'),
                   Line2D([0], [0], lw=0, color='tab:blue', marker='s', label='LCQ-25'),
                   Line2D([0], [0], lw=0, color='tab:orange', marker='s', label='LCQ-50'),
                   Line2D([0], [0], lw=0, color='tab:green', marker='s', label='LCQ-100')]
plt.legend(handles=legend_elements, loc='best')
plt.tight_layout()
plt.savefig(f'./fingerprinting/plots/RQ3_bar.png', bbox_inches='tight')
